...
import 'models/ny_query_parameters.dart';
import 'models/nyrouter_route_guard.dart';
import 'models/nyrouter_options.dart';
...
typedef NyRouteView = Widget Function(BuildContext context);
...
class NyRouter {
  NyRouter({this.options = const NyRouterOptions()}) {
  ...
  void _log(String message) {
    if (options.isLoggingEnabled) {
      NyLogger.debug('[NyRouter] $message');
    }
  }
  ...
  /// Check if the route is a named argument route.
  /// Returns true if the name matches a route pattern with parameters.
  ...
  void group(
    ...
    if (routeConfig.containsKey('route_guards') &&
        routeConfig['route_guards'] is List<RouteGuard>) {
      routeGuards = routeConfig['route_guards'] as List<RouteGuard>;
    }
  ...
  NyRouterRoute? _findRouteForName(String name) {
    final String normalized = _normalizeRouteName(name);
    if (_routeNameMappings.containsKey(normalized)) {
      return _routeNameMappings[normalized];
    }
    final RouteMatch? match = RouteMatcher(
      normalized,
    ).findMatch(_routeNameMappings);
    return match?.route;
  }
  ...
  /// Parses route settings into normalized components.
  /// Returns a record containing the parsed route name, arguments wrapper, and
  /// whether the route is a named argument route.
  ({String routeName, ArgumentsWrapper arguments, bool isNamedArg})
  _parseRouteSettings(RouteSettings settings) {
    ...
    if (uriSettingName != null && uriSettingName.queryParameters.isNotEmpty) {
      argumentsWrapper.queryParameters = NyQueryParameters(
        uriSettingName.queryParameters,
      );
    }
    ...
    if (routeNamedArg) {
      ...
      if (argumentsWrapper.queryParameters != null) {
        Map<String, String> mappedData = Map.from(
          argumentsWrapper.queryParameters!.data,
        );
